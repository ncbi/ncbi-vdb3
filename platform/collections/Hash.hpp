...
inline static uint64_t hashmix(uint64_t hash, uint64_t high, uint64_t low, uint64_t mul )
{
    uint64_t masked=low & 0xFCULL;
    uint64_t h1=(high+masked)*mul;
    uint64_t h2=uint64_ror(h1,47U);
...
uint64_t Hash (int i)
{
    return Hash(&i,sizeof(i));
}

uint64_t Hash (unsigned long long l)
{
    return Hash(&l,sizeof(l));
}
